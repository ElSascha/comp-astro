...
double viscosity(double density, double u_p, double u_m) {
    // Neumann-Richtmyer viscosity
    if(u_p <= u_m){
        return 0.25*pow(xi,2)*pow(u_p - u_m,2)*density;
    }
    else{
        return 0.0;
    }
...
void upwind_advection_step(vector<double> &density, vector<double> &momentum,vector<double> &total_energy) {
    ...
    for (int i = ghost_cells; i < grid_size + ghost_cells; ++i) {
        ...
        double p_half_neg = p(density[i - 1], epsilon) + viscosity(density[i-1],momentum[i]/density[i],momentum[i-2]/density[i-2]); // Pressure at "half-step" in negative direction
    ...
    for (int i = ghost_cells; i < grid_size +ghost_cells; ++i) {
        ...
        double pressure_p = p(density_half[i+1], epsilon_p) + viscosity(density_half[i+1], momentum_half[i+2]/density_half[i+2],momentum_half[i]/density_half[i]); // Pressure at next cell
        double pressure_m = p(density_half[i-1], epsilon_m) + viscosity(density_half[i-1], momentum_half[i]/density_half[i],momentum_half[i-2]/density_half[i-2]); // Pressure at previous cell
        density[i] = density_half[i];
        momentum[i] = momentum_half[i] - (dt / (2*dx))*(pressure_p - pressure_m);
        total_energy[i] = total_energy_half[i] - (dt/(2*dx)) * (pressure_p * u_half_p - pressure_m * u_half_m);
    }
}
...
int main() {
    ...
    filename << "../non_isothermal_flow_viscosity/output/non_isothermal_flow_viscosity.txt";
